...
function summarize(event) {
  const text = event.type === "tool" ? JSON.stringify(event.args || {}) : String(event.text || "");
...
function createTrajectoryView(store) {
  ...
  function renderInspector() {
    ...
    const content = event.text ? `<h3>${event.type === "result" ? "Result" : "Content"}</h3>${conversationalContent(event)}` : "";
  ...
  function render() {
    ...
    $("#trajectoryEvents").innerHTML = visible.map(
      (event) => `
            <button
