...
import { WorkbenchClient, type Run } from "@kyoot/workbench-protocol";
import { makeTypedApi, type WorkbenchBackend } from "../src/typed-api.ts";
...
const backend: WorkbenchBackend = {
  ...
  createRun: async ({ project, prompt, title }) => {
    const run: Run = { id: `r${runs.size + 1}`, project, prompt, title, status: "queued", createdAt: now, updatedAt: now };
    runs.set(run.id, run);
    return run;
  },
...
test("typed Workbench RPC and sync collections share one backend", async () => {
  ...
  const sync = make(fromRpc(client(SyncRpc), syncTransport), { reconnectMs: 1 });
  await sync.start();

  const run = await Kyoot.runPromise(
    WorkbenchClient.runs.create({ project: "kyoot", prompt: "test" }).pipe(
      provide(rpcTransport),
      Fail.orThrow,
    ),
  );
  await new Promise((resolve) => setTimeout(resolve, 10));
  assert.equal(sync.collection<Run>("runs").get(run.id)?.prompt, "test");
  const live = await Kyoot.runPromise(
    WorkbenchClient.live.session({ messages: true, trajectory: false }).pipe(
      provide(rpcTransport),
      Fail.orThrow,
    ),
  );
  ...
  await Kyoot.runPromise(
    WorkbenchClient.files.write({ project: "kyoot", path: "README.md", content: "next" }).pipe(
      provide(rpcTransport),
      Fail.orThrow,
    ),
  );
