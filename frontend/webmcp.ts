...
const integer = (description: string, minimum = 1, maximum = 500) => ({
  type: "integer",
  description,
  minimum,
  maximum,
});
const enumString = (description: string, values: readonly string[]) => ({
  type: "string",
  description,
  enum: values,
});
const limit = (value: unknown, fallback: number, maximum: number) =>
  Math.max(1, Math.min(maximum, Number(value) || fallback));
...
const webMcpCreator = Object.freeze({
  id: "webmcp",
  username: "WebMCP",
  pfp: null,
});
const webMcpAttribution = { creator: webMcpCreator };
...
const workflowSummary = (workflow: any) => ({
  id: workflow.id,
...
const requireProject = (store: any, project: string) => {
  const found = store.projects.get(project);
  if (!found) throw new Error(`Unknown project: ${project}`);
  return found;
};
const requireRun = (store: any, id: string) => {
  const found = store.runs.get(id);
  if (!found) throw new Error(`Unknown run: ${id}`);
  return found;
};

export async function registerWorkbenchWebMcp(
  store: any,
  actions: { navigate(path: string): void; currentProject(): string },
  ...
  const tools: ModelContextTool[] = [
    ...
    },
    {
      ...
      description:
        "List concise coding-agent run summaries. Use workbench_get_run for transcript details.",
      inputSchema: objectSchema({
      ...
      annotations: readOnly,
      execute: ({ project, status, limit: requested }) =>
        store.runs
          .all()
          .filter(
            (run: any) =>
          ...
          .slice(0, limit(requested, 25, 100))
          .map(runSummary),
    },
    {
      ...
      description:
        "Get one coding-agent run. Set includeEvents false for a compact record.",
      inputSchema: objectSchema(
        {
          id: text("Run ID"),
    ...
    },
    {
      ...
      description:
        "Start a coding-agent run and return its authoritative/optimistic Workbench record.",
      inputSchema: objectSchema(
        {
    ...
    },
    {
      ...
      description:
        "Send a follow-up instruction to an existing coding-agent run.",
      inputSchema: objectSchema(
    ...
    },
    {
      ...
      description:
        "Compact an agent run's context so it can continue with a smaller context window.",
      inputSchema: objectSchema({ id: text("Run ID") }, ["id"]),
      annotations: mutation,
      execute: ({ id }) => {
        requireRun(store, id);
    ...
    },
    {
      ...
      description:
        "List a bounded section of the project file tree. Use prefix to scope large repositories.",
      inputSchema: objectSchema(
        {
      ...
      annotations: readOnly,
      execute: async (
        { project, prefix = "", limit: requested },
        { signal } = {},
      ) => {
        requireProject(store, project);
        const files = await abortable(signal, () =>
          store.fileTree(project).load(),
        );
        const matching = prefix
          ? files.filter((file: any) => file.path.startsWith(prefix))
          : files;
        return {
    ...
    },
    {
      ...
      description:
        "Fuzzy-search project filenames for quick-open. This does not search file contents.",
      inputSchema: objectSchema(
        {
          project: text("Project ID"),
    ...
    },
    {
      ...
      description:
        "Read a text file with its revision and metadata. Binary files return metadata without inline bytes.",
      inputSchema: objectSchema(
        {
          project: text("Project ID"),
    ...
    },
    {
      ...
      description:
        "Write complete text content using revision-safe optimistic Workbench mutation handling.",
      inputSchema: objectSchema(
        {
          project: text("Project ID"),
          path: text("Project-relative file path"),
    ...
    },
    {
      ...
      description:
        "Get branch, working-tree status, recent commits, and optional commit detail.",
      inputSchema: objectSchema(
    ...
    },
    {
      ...
      description:
        "Spawn a durable coding agent in a registered project. Alias of workbench_create_run with explicit agent terminology.",
      inputSchema: objectSchema(
        {
    ...
    ...platformTools(store, readOnly, mutation),
    {
      ...
      description:
        "Navigate the visible UI to a Workbench route beginning with /.",
      inputSchema: objectSchema(
  ...
  return [
    {
      ...
      title: "Get workflow",
      description:
        "Get a durable workflow including every task and current status.",
    ...
    },
    {
      ...
      description:
        "Spawn a durable multi-agent DAG. Independent tasks fan out; dependsOn edges create joins.",
      inputSchema: objectSchema(
        {
    ...
    },
    {
      ...
      annotations: { ...mutation, destructiveHint: true },
      execute: ({ workflowId, taskId }) =>
        store.cancelWorkflowTask(workflowId, taskId),
    },
    {
      ...
      annotations: mutation,
      execute: ({ workflowId, taskId }) =>
        store.retryWorkflowTask(workflowId, taskId),
    },
    {
      name: "workbench_workflow_events",
      title: "Read workflow events",
      description:
        "Read cursor-based collective or per-task workflow progress events.",
      inputSchema: objectSchema({
        after: { type: "integer", minimum: 0 },
        workflowId: text("Optional workflow ID"),
        taskId: text("Optional task ID"),
        limit: integer("Maximum events", 1, 5000),
      }),
      annotations: readOnly,
...
function platformTools(
  ...
  return PLATFORM_OPERATION_CATALOG.map((definition: any) => ({
