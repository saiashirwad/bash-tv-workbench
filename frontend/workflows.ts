...
const summary = (workflow) => {
  ...
  return `${counts.completed || 0}/${Object.keys(workflow.tasks || {}).length} done · ${counts.running || 0} running · r${workflow.revision}`;
};

export function createWorkflowView(store: any) {
  ...
  const renderList = () => {
    const workflows = [...store.workflows.all()].sort((a, b) =>
    ...
    $("#workflowList").innerHTML = workflows.length
      ...
      : '<div class="agentempty">NO WORKFLOWS</div>';
  ...
  const renderEvents = () => {
    ...
    output.textContent = events.filter((event) => event.type !== "pi.message_update").map((event) => {
      ...
      return `${event.cursor} ${event.at} ${scope} ${event.type}${event.data === undefined ? "" : ` ${text(event.data)}`}`;
    }).join("\n");
  ...
  const watch = async () => {
    ...
    try {
      ...
      events.push({ cursor: 0, at: new Date().toISOString(), type: "stream.error", data: error.message });
      renderEvents();
    }
  };
  const renderDetail = () => {
    ...
    const tasks = Object.values(workflow.tasks || {}) as any[];
